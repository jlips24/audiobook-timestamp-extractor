...
class AudioAnalyzer:
    ...
    def _ensure_model_available(self):
        ...
        if self.use_mlx:
            ...
            local_model_dir = os.path.join(
                self.models_dir, "mlx-community", f"whisper-{self.model_size}-mlx"
            )
    ...
    def find_chapter_linear(
        self,
        chapter: Chapter,
        start_search_time: float,
        max_search_duration=2700,
        min_confidence=90
    ) -> bool:
        """
        ...
        logger.info(
            f"Scanning Chapter {chapter.index} starting at {seconds_to_hms(int(current_time))}..."
        )
        ...
        while current_time < search_limit:
            ...
            # Extract Chunk
            logger.info(
                f"Scanning chunk {chunk_count}: "
                f"{seconds_to_hms(int(current_time))} - {seconds_to_hms(int(end_time))}..."
            )
            ...
            result = None
            if self.use_mlx:
                # Map model size to mlx-community repo if needed, or let it auto-resolve standard
                # names mlx_whisper usually handles standard names by mapping to default mlx
                # conversions
                try:
            ...
            # Ensure compatibility: mlx output might behave slightly differently but usually
            # returns dict
            ...
            for segment in segments:
                ...
                if p_ratio > 80:
                    logger.debug(
                        f"Candidate: '{text[:20]}...' | Partial: {p_ratio} | "
                        f"Start: {s_ratio} | Final: {int(final_score)}")

                if final_score > 60:
                    logger.debug(
                        f"Match {int(final_score)}% at {int(current_time + segment['start'])}s: "
                        f"{text[:30]}...")
            ...
            if best_ratio >= min_confidence:
                ...
                logger.info(
                    f"CONFIRMED Chapter {chapter.index} at "
                    f"{seconds_to_hms(int(best_segment_time))} (Score: {best_ratio:.2f})")
        ...
        logger.warning(
            f"FAILED Chapter {chapter.index} - Not found after scanning "
            f"{(current_time - start_search_time) / 60:.1f} min."
        )
