...
class TestOutputManager(unittest.TestCase):
    ...
    def test_save_results_basic(self, mock_json_dump, mock_file, mock_mkdir):
        """
        ...
        # Should not be in output normally if filtered before, but save_results receives
        # 'valid_chapters'

        # NOTE: save_results loop processes all given chapters. If logic says "status == FOUND" it
        # adds time. If not found, it adds empty string.
        ...
        # Since we mocked mkdir on the Path object instance created inside the function, checking
        # calls is tricky directly on the class. But we can assume it works if no error.
    ...
    @patch('src.output_manager.open', new_callable=mock_open)
    @patch('src.output_manager.json.dump')
    def test_save_results_intro_insertion(
        self,
        mock_json_dump,
        mock_file,
        mock_mkdir
